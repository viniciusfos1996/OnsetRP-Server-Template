    ...
    if shop then
        ...
        if dist < 250 then
            if shop.category == "weapons" and PlayerData[player].gun_license == 0 then
                CallRemoteEvent(player, "MakeNotification", _("no_gun_license"), "linear-gradient(to right, #a03b44, #a03b44)")
            else
...
AddRemoteEvent("ShopBuy", function(player, shopid, item, amount)
    ...
    if GetPlayerCash(player) < itemPrice then
        CallRemoteEvent(player, "MakeNotification", _("not_enought_cash"), "linear-gradient(to right, #7f2f36, #7f2f36)")
    else        
        if AddInventory(player, item.name, amount) == true then
            RemovePlayerCash(player, itemPrice)
            CallRemoteEvent(player, "MakeNotification", _("shop_success_buy", tostring(amount), _(item.name), _("price_in_currency", itemPrice)), "linear-gradient(to right, #3c958a, #3c958a)")
        else
            CallRemoteEvent(player, "MakeNotification",_("inventory_not_enough_space"), "linear-gradient(to right, #7f2f36, #7f2f36)")
        end
    end
end)
...
AddRemoteEvent("ShopSell", function(player, shopid, item, amount) 
    ...
    if tonumber(PlayerData[player].inventory[itemName]) < tonumber(amount) then
        CallRemoteEvent(player, "MakeNotification", _("not_enough_item"), "linear-gradient(to right, #8e343c, #8e343c)")
    else
        AddPlayerCash(player, math.ceil(itemPrice))
        CallRemoteEvent(player, "MakeNotification", _("shop_success_sell", tostring(amount), _(itemName), _("price_in_currency", itemPrice)), "linear-gradient(to right, #329589, #329589)")
